...
// ==========================================
// SISTEMA DE BADGE (ICONO ESCRITORIO Y PESTAÑA)
// ==========================================
let totalMensajesNoLeidos = 0;

function actualizarBadge(cantidad) {
    totalMensajesNoLeidos = cantidad;
    if (cantidad > 0) {
        // 1. Badging API nativa (PWA en Windows/Mac/Android)
        if ('setAppBadge' in navigator) {
            navigator.setAppBadge(cantidad).catch(() => {});
        }
        // 2. Título de la pestaña del navegador
        document.title = `(${cantidad}) ConectaVzla`;
        // 3. Fallback visual: Dibujar número sobre el favicon
        actualizarFaviconBadge(cantidad);
    } else {
        limpiarBadge();
    }
}
...
function limpiarBadge() {
    ...
    document.title = 'ConectaVzla';
    restaurarFaviconOriginal();
}

function actualizarFaviconBadge(cantidad) {
    const canvas = document.createElement('canvas');
    canvas.width = 32; canvas.height = 32;
    const ctx = canvas.getContext('2d');
    const img = new Image();
    img.src = 'img/android-chrome-192.png'; // Tu favicon actual
    ...
    img.onload = () => {
        ...
        const texto = cantidad > 99 ? '99+' : cantidad.toString();
        ctx.fillText(texto, 24, 8.5);
...
function restaurarFaviconOriginal() {
    let link = document.querySelector("link[rel*='icon']");
    if (link) link.href = 'img/android-chrome-192.png';
}
// ==========================================
...
async function cargarContactos() {
    try {
        ...
        // === NUEVO: Calcular y actualizar el badge global ===
        const totalNoLeidos = allContacts.reduce((acc, curr) => acc + (curr.unread || 0), 0);
...
async function abrirChat(contactName, chatId, otherId) {
    ...
    // === NUEVO: Limpiar badge del contacto que se está abriendo ===
    const contactoAbierto = allContacts.find(c => c.id === currentChatId);
    if (contactoAbierto && contactoAbierto.unread > 0) {
        contactoAbierto.unread = 0; // Lo marcamos como leído en memoria
        const totalNoLeidos = allContacts.reduce((acc, curr) => acc + (curr.unread || 0), 0);
        actualizarBadge(totalNoLeidos);
    }
...
async function abrirGrupo(groupId, groupName, adminId) {
    ...
    // === NUEVO: Limpiar badge del grupo que se está abriendo ===
    const grupoAbierto = allContacts.find(c => c.id === currentGroupId);
    if (grupoAbierto && grupoAbierto.unread > 0) {
        grupoAbierto.unread = 0;
        const totalNoLeidos = allContacts.reduce((acc, curr) => acc + (curr.unread || 0), 0);
        actualizarBadge(totalNoLeidos);
    }
